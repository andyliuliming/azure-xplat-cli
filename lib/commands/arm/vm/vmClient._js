...
var BlobUtilities = azureStorage.BlobUtilities;
...
__.extend(VMClient.prototype, {
    ...
    if (!params.disableBginfoExtension && osType.toString().toLowerCase() == 'windows') {
      ...
      for (var key1 in publishers) {
        if (publishers[key1].name === vmConstants.EXTENSIONS.BGINFO_PUBLISHER) {
          var typeResult = vmImage.getVMExtensionImageTypeList(location, publishers[key1].name, _);
          var types = typeResult.resources;
          for (var key2 in types) {
            if (types[key2].name === vmConstants.EXTENSIONS.BGINFO_NAME) {
              var versionResult = vmImage.getVMExtensionImageVersionList(location, publishers[key1].name, types[key2].name, _);
              var versions = versionResult.resources;
  ...
  createVMBackup: function (resourceGroupName, name, options, _) {
    ...
    var storageBlobUris = [];
    storageBlobUris.push(vmResult.virtualMachine.storageProfile.oSDisk.virtualHardDisk.uri);
    var dataDisks = vmResult.virtualMachine.storageProfile.dataDisks;
    dataDisks.forEach(function (dataDisk) {
        storageBlobUris.push(dataDisk.virtualHardDisk.uri);
    });
    ...
    var result = virtualMachine.createOrUpdateVMExtension(resourceGroupName, name, azureVmBackupExtension.profile, true, _);
        
    if (!(result) ||
    !utils.ignoreCaseEquals(result.status , vmConstants.EXTENSIONS.EXTENSION_PROVISIONING_SUCCEEDED)) {
        throw new Error(util.format($('call extension %s failed with : %s  '), vmConstants.AZURE_VM_BACKUP_LINUX_EXTENSION_PUBLISHER, result.error.message));
    }
        ...
        if (result.accountName && result.containerName && result.blobName) {
            ...
            for (var i = 0, len = blobs.length; i < len; i++) {
                ...
                if (blob.metadata["vmbackuptag"] == options.identity) {
                    ...
                    var tips = util.format($('Deleting Blob %s in container %s'), blob.name, result.containerName);
                    var operation = this._getStorageBlobOperation(blobService, 'deleteBlob');
                    startProgress(tips);
                    try {
  ...
  setAzureDiskEncryption: function (resourceGroupName, name, options, _) {
    ...
    if (utils.ignoreCaseEquals(vmResult.virtualMachine.storageProfile.oSDisk.operatingSystemType, "Linux")) {
    ...
    if (!utils.ignoreCaseEquals(vmResult.virtualMachine.storageProfile.oSDisk.operatingSystemType, "Linux")) {
        if (options.passphrase) {
            throw new Error($('only Linux support the passphrase specified in parameters.'));
        }
    } else {
    ...
    var extensionResult = serviceClients.computeManagementClient.virtualMachineExtensions.getWithInstanceView(resourceGroupName, name, extensionName, _);

    if(!extensionResult ||
       !extensionResult.virtualMachineExtension || 
       !extensionResult.virtualMachineExtension.instanceView ||
       !extensionResult.virtualMachineExtension.instanceView.statuses ||
       (extensionResult.virtualMachineExtension.instanceView.statuses.length < 1) ||	   
       !utils.ignoreCaseEquals(extensionResult.virtualMachineExtension.publisher , publisherName) ||
       !utils.ignoreCaseEquals(extensionResult.virtualMachineExtension.extensionType , extensionName) ||
       !utils.ignoreCaseEquals(extensionResult.virtualMachineExtension.provisioningState , vmConstants.EXTENSIONS.EXTENSION_PROVISIONING_SUCCEEDED))
    {
    ...
    var statusUrl = extensionResult.virtualMachineExtension.instanceView.statuses[0].message;
    ...
    if (!vmModel ||
        !vmModel.virtualMachine ||
        !vmModel.virtualMachine.storageProfile) 
    {
    ...
    return virtualMachine.createOrUpdateVM(resourceGroupName, vmModel.virtualMachine, false, _);
  },
  ...
  showAzureDiskEncryptionStatus: function(resourceGroupName, name, options, _) {
    ...
    if(vmModel.virtualMachine.storageProfile.oSDisk.encryptionSettings)
    {
      ...
      else if(utils.ignoreCaseEquals(vmModel.virtualMachine.storageProfile.oSDisk.operatingSystemType, 'Windows')) {
          extensionName = vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_NAME;
          publisherName = vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_PUBLISHER;
      }

      try {	  
        extensionStatus = serviceClients.computeManagementClient.virtualMachineExtensions.getWithInstanceView(resourceGroupName, name, extensionName, _);
      }
      catch (e) {
      ...
      if((extensionStatus) &&
         (extensionStatus.virtualMachineExtension) && 
         utils.ignoreCaseEquals(extensionStatus.virtualMachineExtension.publisher , publisherName) &&
         utils.ignoreCaseEquals(extensionStatus.virtualMachineExtension.extensionType , extensionName) &&
         utils.ignoreCaseEquals(extensionStatus.virtualMachineExtension.provisioningState , vmConstants.EXTENSIONS.EXTENSION_PROVISIONING_SUCCEEDED))
      {
        if(!extensionStatus.virtualMachineExtension.settings.VolumeType ||
           utils.ignoreCaseEquals(extensionStatus.virtualMachineExtension.settings.VolumeType , 'All') ||
           utils.ignoreCaseEquals(extensionStatus.virtualMachineExtension.settings.VolumeType , 'Data') ||
           (utils.stringIsNullOrEmpty(extensionStatus.virtualMachineExtension.settings.VolumeType)))
        {
