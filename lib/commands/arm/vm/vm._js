...
exports.init = function (cli) {
  ...
  vm.command('enable-disk-encryption [resource-group] [name]')
    ...
    .option('-h, --passphrase <passphrase>', $('Passphrase specified in parameter'))
    .option('-q, --quiet', $('quiet mode, do not ask for confirmation'))
    .option('-e, --extension-version <extension-version>', util.format($('Version of Diagnostics extension. Default values are [%s] for Windows'), vmConstants.EXTENSIONS.AZURE_DISK_ENCRYPTION_WINDOWS_EXTENSION_VERSION))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, name, options, _) {
      resourceGroup = cli.interaction.promptIfNotGiven($('Resource group name: '), resourceGroup, _);
      name = cli.interaction.promptIfNotGiven($('Virtual machine name: '), name, _);
  ...
  vm.command('remove-backup [resource-group] [name]')
    .description($('Remove the backup for the vm'))
    .usage('[options] <resource-group> <name>')
